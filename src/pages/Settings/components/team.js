...
const photoImgClasses = member => {
	return ClassNames({
		[styles.photo]: true,
		[styles.photo_null]: member.isWaiting || !member.user.avatar,
	});
};
...
class Team extends Component {
	...
	render() {
		...
		if (!checkPermissions(currentUserRole, ['studio.control'])) return null;
		else
			return (
				<CardPaper
					elevation={1}
							...
							!member.isWaiting ? (
								<div className={styles.item} key={member._id}>
									<div className={photoImgClasses(member)}>
										{member.user.avatar ? <img src={member.user.avatar} alt="" /> : <FontAwesomeIcon icon={['fas', 'user-alt']} />}
									</div>
									...
									{member.role !== 'owner' || (member.role === 'owner' && checkPermissions(currentUserRole, ['studio.full_control'])) ? (
										<IconButton
