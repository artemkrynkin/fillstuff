...
const Autocomplete = forwardRef(function Autocomplete(props, ref) {
	...
	const {
		...
		multiple = false,
		noOptionsText = <Typography variant="caption">Нет вариантов для выбора</Typography>,
		onChange,
		onClose,
		onHighlightChange,
		onInputChange,
	...
	if (multiple && value.length > 0) {
		...
		if (renderTags) {
			...
			startAdornment = value.map((option, index) => (
				<Chip label={getOptionLabel(option)} size={size} {...getCustomizedTagProps({ index })} {...ChipProps} />
			));
		}
	}
	...
	const renderListOption = (option, index) => {
		...
		const disabled = optionProps['aria-disabled'];

		return (
			<MenuItem className={classes.option} {...optionProps} {...optionOtherProps} selected={selected} disabled={disabled}>
				{renderOption(option, {
							...
							<div className={classes.endAdornment}>
								...
								{hasClearIcon ? (
									<IconButton
										...
										title={clearText}
										className={ClassNames(classes.clearIndicator, {
								...
								{hasPopupIcon ? (
									<IconButton
										...
										className={ClassNames(classes.popupIndicator, {
				...
				>
					<PaperComponent className={classes.paper}>
						{groupedOptions.length > 0 ? (
							<ListboxComponent className={ClassNames(classes.listbox)} {...getListboxProps()} {...ListboxProps}>
						...
						{loading && groupedOptions.filter(option => !option.itemCreation).length === 0 ? (
							<div className={classes.loading}>{loadingText}</div>
						) : null}
						{groupedOptions.filter(option => !option.itemCreation).length === 0 && !freeSolo && !loading ? (
